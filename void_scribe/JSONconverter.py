...
for jsonFile in os.listdir(data_path):
    ...
    newJSON = copy.deepcopy(jsonData)
    for componetID in jsonData['componets'].keys():
        newJSON['componets'][componetID] = jsonData['componets'][componetID][0]

    with open(data_path + jsonFile, 'w') as outfile:
        json.dump(newJSON, outfile)
